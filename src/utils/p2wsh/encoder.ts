 ...
 * This is a modified version of the original CIP33 implementation to run in TS
 * https://github.com/Jpja/Electrum-Counterparty/blob/master/js/xcp/cip33.js
 */

// biome-ignore lint/complexity/noStaticOnlyClass: <explanation>
export class P2WSHEncoder {
    ...
    static p2wsh_encode_hex(file_hex: string, network = "bitcoin"): string[] {
        ...
        for (const element of chunks) {
            addresses.push(P2WSHEncoder.cip33_hex_to_bech32(element, network));
        }
    ...
    static p2wsh_decode_hex(addresses: string[]) {
        /*
        ...
        for (const element of addresses) {
            recreated += P2WSHEncoder.cip33_bech32toHex(element);
        }
        let recr_size: string | number = recreated.substring(0, 4);
        recr_size = Number.parseInt(recr_size, 16);
    ...
    static cip33_hex_to_bech32(hex: string, network = "bitcoin") {
        ...
        //Calculate checksum
        const chk = P2WSHEncoder.cip33_bech32_checksum(hrp, intArray);
    ...
    static cip33_bech32_checksum(hrp: string, data: number[]) {
        const values = P2WSHEncoder.cip33_hrpExpand(hrp).concat(data).concat([
            0,
            0,
            0,
            0,
        ...
        const mod = P2WSHEncoder.cip33_polymod(values) ^ 1;
        const ret = [];
        for (let p = 0; p < 6; ++p) {
    ...
    static cip33_bech32toHex(address: string) { //https://en.bitcoin.it/wiki/Bech32
        ...
        for (const element of bin8) {
            hex += Number.parseInt(element, 2).toString(16).padStart(2, "0");
        }
        return hex;
    }
}

export default P2WSHEncoder;